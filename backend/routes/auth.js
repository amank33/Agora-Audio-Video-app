...
router.post('/login/host', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: host._id, type: 'host',data:host }, process.env.JWT_SECRET);
...
router.post('/login/user', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, type: 'user',data:user }, process.env.JWT_SECRET);
